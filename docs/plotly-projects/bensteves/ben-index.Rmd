...
plot_ly(data=us_co2, 
  ...
  layout(title = "USA CO2 Consumption - Fuel Outputs",
         ...
         xaxis = list(title = "Year", tickangle=45, tickfont = list(size=10)),
...
output2 <-tagList(lapply(df_list, show_maps, id_code=id_code2, 
                  map_title=map_title2, years=years, color="RdBu", b= F))
output2
```
...
elec_perc <- world_climate %>%
  filter(year %in% years, 
         indicator_code == id_code2) %>%
          ...
          select(country_name, `1997`, `2007`, `2017`) %>%
          filter(!is.na(`1997`)) %>%
          rename(Country = "country_name")

elec_perc_cond <- elec_perc %>%
   mutate(highlight = ifelse(`2017` >= 25.0 |
                              `1997` >= 25.0 |
                               `2007` >= 25.0, "#90EE90", "white"))
...
plot_ly(data=elec_perc,
       ...
       # columnorder = c(1,2,3,4),
        ...
        cells = list(values=
                      ...
                      fill = list(color = list('rgb(176, 200, 232)', elec_perc_cond$highlight)),
